...
_.filter = function (collection, test){
    ...
    // > of the test, determine if value has passed the test//
...
_.partition = function(array, test){
    var truthyArr = _.filter(array, test);
    var falseyArr = _.reject(array, test);
    var partitionedArr = [];
    partitionedArr.push(truthyArr);
    partitionedArr.push(falseyArr);
...
_.map = function(collection, action){
    var returnValues = [];
    _.each(collection, function(element , pos, index){returnValues.push(action(element, pos, index))});
    return returnValues;
};
...
_.contains = function(array, value){
    var result = false;
    _.each(array, function(element, pos, collection){element ===value ? result = true: false });
    return result;
};
...
_.every = function(collection, test){
    var mapped;
    if(test === undefined ){
        mapped = _.map(collection, function(value, pos, collection){return Boolean(value);});
    }else{
